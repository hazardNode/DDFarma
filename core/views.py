    ...
    if request.method == 'POST':
            ...
            if email:
                if EmailAddress.objects.filter(email=email).exists():
                    ...
                    messages.success(request,
                                     f'Correo de verificación enviado a {email}.')
        ...
        # Handle setting an email as primary
            ...
            if not emailaddress.verified:
                messages.error(request,
                               'No puedes establecer un correo no verificado como principal.')
            else:
                emailaddress.set_as_primary()
                messages.success(request,
                                 f'{email} es ahora tu dirección de correo principal.')
        ...
        # Handle resending verification email
            ...
            try:
                ...
                messages.success(request,
                                 f'Correo de verificación reenviado a {email}.')
                return redirect('account_email_verification_sent')
            ...
            try:
                ...
                if email_address.primary:
                    if EmailAddress.objects.filter(user=request.user).count() > 1:
                        ...
                        messages.error(request,
                                       'No puedes eliminar tu única dirección de correo.')
                        return redirect('account_email')
...
def checkout(request):
    ...
    if len(cart) == 0:
        messages.warning(request, "Tu carrito está vacío. Por favor agrega productos antes de proceder al checkout.")
        return redirect('cart_detail')
...
def checkout_complete(request):
    ...
    if not checkout_data:
        messages.warning(request, "Información de checkout no encontrada.")
        return redirect('checkout')
